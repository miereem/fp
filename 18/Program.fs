...
//             |> List.rev
//             |> (fun rows ->
//                 let lastRow = List.head rows
//                 let secondToLastRow = List.head (List.tail rows)
//                 let newRow =
//                     List.mapi (fun i x -> x + max lastRow.[i] lastRow.[i+1]) secondToLastRow
//                 newRow :: List.tail (List.tail rows) |> List.rev)
//         maxPathSum updatedTriangle

// let result = maxPathSum triangle
// printfn "The maximum total from top to bottom is: %d" result
